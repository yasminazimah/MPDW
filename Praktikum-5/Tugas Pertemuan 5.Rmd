...
Gambar di atas menunjukkan nilai *rounded value* ($\lambda$) optimum sebesar **0,25** dan pada selang kepercayaan 95% nilai memiliki batas bawah **0,00** dan batas atas **1,33**. Selang tersebut memuat nilai satu sehingga dapat dikatakan bahwa data bangkitan stasioner dalam ragam.
...
mean(dt_2)
```
Plot deret waktu di atas menunjukkan bahwa data stasioner dalam rataan, ditandai dengan data yang menyebar di sekitar nilai tengahnya (17.94844) dan stasioner dalam ragam, ditandai dengan lebar pita yang cenderung sama.
...
```{r}
index <- seq(1:122)
bc = boxcox(dt_2~index, lambda = seq(0,6,by=1))
#Nilai Rounded Lambda
lambda <- bc$x[which.max(bc$y)]
lambda
#SK
...
Gambar di atas menunjukkan nilai *rounded value* ($\lambda$) optimum sebesar **2.30303** dan pada selang kepercayaan 95% nilai memiliki batas bawah **0.00** dan batas atas **4.90909091**. Selang tersebut memuat nilai satu sehingga dapat dikatakan bahwa data bangkitan stasioner dalam ragam.
