...
```
## Input Data
Data yang digunakan adalah data CO2 Emissions from Fossil Fuels di United Kingdom pada tahun 1968-2014. Dengan peubah y yaitu jumlah Bunker fuels, kemudian x1 yaitu Tahun, x2 yaitu Emisi karbon per capita, dan X3 yaitu jumlah gas flaring yang dihasilkan.
```{r}
library(rio)
...
summary(model)
```
$$y_i = -596000+296.9x1_t+5010x2_t-0.4298x3_t$$
...
Selanjutnya dapat dilihat juga nilai $R^2=0.8057$. Artinya, sebesar 80.57% keragaman jumlah Bunkel fuels dapat dijelaskan oleh peubah Tahun, Per capita, san Gas flaring. Hasil ini menunjukkan hasil yang bagus, seolah mendapatkan hasil terbaik. Namun, kita perlu melakukan uji terhadap sisaannya seperti berikut ini.
...
abline(a = 0, b = 0, lwd = 2)
```
Normal Q-Q Plot yang dihasilkan menunjukkan bahwa sisaan cenderung menyebar normal. Plot Sisaan vs Fitted Value dan plot antara sisaan vs order menunjukkan adanya autokorelasi karena terdapat pola antar sisaan. 
...
dwtest(model)
```
Berdasarkan hasil Durbin-Watson Test, didapatkan nilai p-value = $2.2e-16$ < alpha 5%, artinya terjadi autokorelasi. Didapatkan juga nilai $DW = 0.29539$. Berdasarkan tabel Durbin-Watson dengan k=3 dan jumlah data 47, diperoleh nilai $DL = 1.3989$ dan $DU = 1.6692$. Nilai DW berada di antara nilai 0 dan DL. Artinya, terjadi autokorelasi positif.
...
dwtest(modelCO)
```
Model menggunakan Cochrane-Orcutt $$y_i=-458639.7+232.7415x1_t+823.1627x2_t+0.001507x3_t$$.

Nilai DW setelah dilakukan penanganan meningkat menjadi $DW = 1.9548$. Nilai DW sudah berada pada rentang DU \< DW \< 4-DU atau $1.6692 < DW < 2.3308$, maka sudah tidak ada autokorelasi. Hal tersebut juga didukung oleh *p-value* sebesar $0.3815$, di mana *p-value* \> $\alpha$=5%. Artinya tak tolak $H_0$ atau belum cukup bukti menyatakan bahwa ada autokorelasi pada taraf nyata 5%.
...
text(x=0.9, y=12478591, labels = "rho=0.9", cex = 0.8)
```
...
```
Setelah dilakukan tranformasi balik, didapatkan model dengan metode Hildreth-Lu sebagai berikut. $$y_i=-456254.8+231.5577x1_t+845.1527x2_t+0.001243035x3_t$$

```{r}
#Deteksi autokorelasi
dwtest(modelHL)
```
Hasil uji Durbin-Watson juga menunjukkan bahwa nilai DW meningkat menjadi $1.9449$.
Nilai DW sudah berada pada rentang DU \< DW \< 4-DU atau $1.6692 < DW < 2.3308$, maka sudah tidak ada autokorelasi positif. Hal tersebut juga didukung oleh *p-value* sebesar $0.3683$, di mana *p-value* \> $\alpha$=5%. Artinya tak tolak $H_0$ atau belum cukup bukti menyatakan bahwa ada autokorelasi pada taraf nyata 5%.
...
akurasi
```
Berdasarkan hasil tersebut dapat diketahui bahwa hasil penanganan autokorelasi dengan metode Cochrane-Orcutt dan Hildreth-Lu memiliki nilai SSE yang tidak jauh berbeda atau cenderung sama, sebesar $12474032.5$ pada metode Cochrane-Orcutt dan $12478592.4$ pada metode Hildreth-Lu. Nilai tersebut lebih baik dibandingkan model awal ketika autokorelasi masih terjadi, yaitu sebesar $69811722$.
